function Y = sym_sne(X, d, labels, perplexity)
%SNE Implementation of symmetric Stochastic Neighbor Embedding
%
%   mappedX = sym_sne(X, no_dims, labels, perplexity)
%
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    max_iter = 12000;                % maximum number of iterations
    ...
    for iter=1:max_iter
        ...
        % Compute cost function between P and Q & Print out progress
        if ~rem(iter, 200)
            ...
            str = ['Result of sym-SNE: ', num2str(iter), ' iterations'];
            if ~isempty(labels)
                if d == 1
                    scatter(Y, 15, labels, 'filled'); title(str);
                    
                elseif d == 2
                     scatter(Y(:,1), Y(:,2), 15, labels, 'filled'); title(str);
                    %  scatter(ydata(:,1), ydata(:,2), 15, 'filled'); title(str); % without lables
                else
                     scatter3(Y(:,1), Y(:,2), Y(:,3), 15, labels, 'filled'); title(str);
