function ydata = gSNE(X, labels, no_dims, initial_dims, perplexity, method, alpha, beta, CGPU_type)
%gSNE Performs symmetric geheral SNE on dataset X
%
%   mappedX = gSNE(X, labels, no_dims, initial_dims, perplexity)
%   mappedX = gSNE(X, labels, initial_solution, perplexity)
%
% The function performs gSNE, arbitrary probability distribution, on the NxD dataset X to reduce its
...
% data are not used by gSNE itself, however, they are used to color
...
if ~exist('CGPU_type', 'var') || isempty(CGPU_type)
    CGPU_type = 'CPU';
end
...
% Run gSNE
if initial_solution
    ydata = gSNE_p(P, labels, ydata, method, alpha, beta, CGPU_type);
else
    ydata = gSNE_p(P, labels, no_dims, method, alpha, beta, CGPU_type);
end
